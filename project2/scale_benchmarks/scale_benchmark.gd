...
func _process(delta):
	...
	# Note: negative indexes don't work in early 2.x
	var last = _frame_stats.size() - 1
	_frame_stats[last]["static_memory"] = static_memory
	_frame_stats[last]["dynamic_memory"] = dynamic_memory
